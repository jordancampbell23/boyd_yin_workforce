...
decrement <- readRDS("generated_data/decrement.rds")
...
init_pop <- readRDS("generated_data/init_pop.rds")
init_sal <- readRDS("generated_data/init_sal.rds")
entrants_dist <- readRDS("generated_data/entrants.rds")
...
# # library("plyr", lib.loc = "/Library/Frameworks/R.framework/Versions/4.1/Resources/old_library")
# 
...
get_Population <- function(.init_pop = init_pop,
  ...
  make_dmat <- function(qx, df = decrement_wf) {
    ...
    # returns:
    # a transtion matrix
    df %<>% select_("age", "ea", qx) %>% ungroup %>% tidyr::spread_("ea", qx, fill = 0) %>% select(-age) %>% t # need to keep "age" when use spread
  ...
  wf_active <- plyr::adply(wf_active, 3, function(x) {df = as.data.frame(x); df$ea = as.numeric(rownames(x));df}) %>% 
    rename(year = X1) %>%
